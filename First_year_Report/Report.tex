...
\begin{figure}[!htp]
\centering
\includegraphics[width=0.8\textwidth]{shift_plot.png}
\caption{\footnotesize{\emph{}}}
...
\subsubsection{Frame Diagnostics}
The quality of the sky subtraction can vary significantly on a frame by frame basis, and so when stacking many reconstructed datacubes to increase S/N for an object, one can carry through the results of poor sky subtractions to the whole stack. It is necessary to check the quality of sky subtraction on a frame by frame basis, by feeding a list of object/sky pairs through the science reconstruction recipe. The spectral locations of strong OH emission lines are found by examining a reconstructed skycube, and the quality of the sky subtraction is found for each IFU by taking a median of the stacked object flux at these wavelengths. Figure \ref{fig:sky_sub_perf} show a summary of this diagnostic for the red supergiant stars. The top panel shows the average sky subtraction performance across all IFUs for each science frame, the middle panel plots for each frame the sky subtraction performance as a function of IFU and the bottom panel drills down to subplots of the sky subtraction performance in each IFU as a fuction of IFU frame. Note that the IFUs are arrange in rows of 8 to reflect the three detectors, and some of the IFUs were not operational during the exposures. From these plots it is easy to determine where things are going wrong and which frames to omit from the stack. For example, in Figure \ref{fig:sky_sub_perf} frame 5 clearly has a far poorer sky subtraction than the rest, and it is an ongoing process for us to understand why this should be the case. Omitting frame 5 from the stack significantly improves the quality of the extracted spectra. 


\begin{figure}[!htp]
\centering
\subfloat[]{\includegraphics[width=0.32\textwidth]{frame_performance.png}}
\subfloat[]{\includegraphics[width=0.32\textwidth]{IFU_by_frame.png}}
\subfloat[]{\includegraphics[width=0.32\textwidth]{IFU_subplots.png}}
\caption{\footnotesize{\emph{The wavelengths of prominent OH emission lines are determined by looking at the spectrum of a skycube. A sky subtraction performance `statistic' is found by taking the median of the object cube flux at these wavelengths. The top panel shows the quality of the sky subtraction as a function of frame, taking an average of the statistic across all IFUs in the frame. The middle panel shows the statistic as a function of IFU, plotted for each of the frames taken. The bottom panel shows the highest level of detail, drilling down to the sky subtraction performance for each frame in each IFU. This plot has the potential to highlight an optimal combination of frames to stack for each object. }}}
\label{fig:sky_sub_perf}
\end{figure}

In a similar vein we also check the PSF of a standard star for each of the frames. During an OB, KMOS dedicates one of the IFUs to observing a standard star, to monitor the variation in its PSF and hence variations in seeing as the night progresses. For the red supergiants, a single bright object is chosen for this analysis. A 2D gaussian is fit to the object image and the FWHM of this gaussian gives an indication of the instantaneous seeing at the time the frame was observed. Figure \ref{fig:fwhm_track} shows the evolution of the FWHM of the standard star on the same night as the data in Figure \ref{fig:sky_sub_perf} were taken.

\begin{figure}[!htp]
\centering
\includegraphics[width=0.7\textwidth]{frame_fwhm.png}
\caption{\footnotesize{\emph{}}}
\label{fig:fwhm_track}
\end{figure}

There does not appear to be any correlation between the seeing and the quality of the sky subtraction. As part of this analysis, objects are divided into four bins of seeing quality. It would be unwise to degrade the quality of an image with good seeing, by stacking an object with bad. The above analysis gives control over how stacking is carried out and which weights should be applied to the different seeing bins.  
